...
### Creating document snippets

After creating the index, you can generate document snippets for search results using the `doc_snippets.py` script:

```bash
pyhton3 src/sapien/tools/doc_snippets.py
```

### Creating offsets to speed up snippet retrieval
To speed up snippet retrieval, you can create offsets for the indexed documents using the `create_offsets.py` script:

```bash
python3 src/sapien/tools/generate_full_docs.py
python3 src/sapien/tools/build_full_doc_offsets.py
```

### How to setup Gemini
Put your Gemini API key in a `.env` file at the root of the project:

```
GEMINI_API_KEY=your_gemini_api_key_here
```

## Search Options in Frontend
The static web interface provides several search options that can be toggled:


<p align="center">
    <img height="100px;" src="./readme-img/options.png">
</p>

### Search
This is the standard keyword-based search that retrieves documents based on exact matches of the query terms.

### Search similar
This option allows users to find documents that are similar in content to the query, even if they do not contain the exact keywords. This options needs the document id to search for similar documents.

### Improved search
This advanced search method leverages natural language processing techniques to understand the context and meaning of the query, providing more relevant results based on semantic similarity rather than just keyword matching.

### Answer (rag)
This feature utilizes Retrieval-Augmented Generation (RAG) to provide direct answers to user queries by combining information retrieval with generative AI models. It retrieves relevant documents and generates concise answers based on the content.
